...
import {
  ...
  StyleSheet, SafeAreaView, TextInput, Image, FlatList,
} from 'react-native';
...
function RecentCard({ item, onPress }) {
  ...
  return (
    <TouchableOpacity style={styles.recentCard} onPress={onPress} activeOpacity={0.92}>
      ...
      <TouchableOpacity style={styles.recentHeart} onPress={() => setSaved(!saved)}>
      ...
      <View style={styles.recentInfo}>
        ...
        {item.beds > 0 && (
          <Text style={styles.recentMeta}>🛏 {item.beds}  🚿 {item.baths}  {item.verified ? '✅' : ''}</Text>
        )}
      </View>
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  ...
  searchBox: {
    ...
    marginBottom: 16, borderWidth: 1, borderColor: '#E5E0D5',
  },
  ...
  statsBanner: {
    flexDirection: 'row', backgroundColor: '#1B4332',
  ...
  featuredCard: {
    width: 260, marginRight: 14, borderRadius: 18, backgroundColor: '#FFFFFF',
  ...
  recentCard: {
    ...
    marginHorizontal: 20, borderRadius: 14, marginBottom: 12,
    shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, elevation: 3,
    overflow: 'hidden', borderWidth: 1, borderColor: '#E5E0D5',
  },
  recentImage: { width: 110, height: 100 },
  recentHeart: { position: 'absolute', top: 8, left: 88, backgroundColor: 'rgba(255,255,255,0.9)', borderRadius: 12, padding: 3 },
